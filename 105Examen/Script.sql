...
-- -----------------------------------------------------
-- Table `mydb`.`CConfiguracion`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`CConfiguracion` ;

CREATE TABLE IF NOT EXISTS `mydb`.`CConfiguracion` (
  `id_config` INT NOT NULL AUTO_INCREMENT,
  `config` VARCHAR(45) NULL,
  `id_marca` INT NOT NULL,
  PRIMARY KEY (`id_config`, `id_marca`),
  INDEX `fk_CConfirguracion_CMarca1_idx` (`id_marca` ASC) VISIBLE)
ENGINE = MyISAM;
...
CREATE TABLE IF NOT EXISTS `mydb`.`MEquipo` (
  ...
  CONSTRAINT `fk_MEquipo_CLab1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_MEquipo_CConfiguracion1`
    FOREIGN KEY (`id_config`)
    REFERENCES `mydb`.`CConfiguracion` (`id_config`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
